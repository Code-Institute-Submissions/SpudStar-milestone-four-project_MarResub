...
The page system also currently has trouble with accessing different pages when searching by name of a pokemon, instead sending the user to the page desired. This is due to conflicting queries from the name and the category and due to time constraints I was unable to fix before testing making it a known bug. Thankfully this rarely comes up as most pokemon name searches will not exceed one page currently unless a singular letter is used therefore it isn't a major problem keeping it in. It is certainly a feature I want to implement in the future however.
